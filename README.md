...
3. **Estimator.** Implement the complementary filter inside `StateEstimator` and validate
   against logged traces in tests.
4. **Open loop motor characterization.** Use `HubMotorTest.py` to map duty to angular
   velocity. Capture data with `DataLogger`.
5. **Inner loop wheel velocity controller.** A trivial proportional loop that lets you
   command wheel velocity from a higher level.
